(function(win) {
	...
	skyScroll.prototype = {
		...
		init: function(opts) {
			...
			var _this = this,
				mousemoveHandle, mousedownHandle;
		...
		recalculated: function() {
			var H = this.target.offsetHeight,
				...
				T = parseInt(this.target.style.top),
				L = parseInt(this.target.style.left),
				h, w;
			this.ratio = {
				l: this.width / W,
				v: this.height / H
	...
	function _getElementPosition(el) {
		...
		if (el.getBoundingClientRect) {
			...
			var d_root = document.documentElement,
				db = document.body;
